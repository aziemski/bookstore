...
func (bq *BookQuery) Exist(ctx context.Context) (bool, error) {
	...
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("entgo: check existence: %w", err)
	default:
		return true, nil
	}
}
...
func (bq *BookQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range bq.inters {
		if inter == nil {
			return fmt.Errorf("entgo: uninitialized interceptor (forgotten import entgo/runtime?)")
		}
	...
	for _, f := range bq.ctx.Fields {
		if !book.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("entgo: invalid field %q for query", f)}
		}
	}
	if bq.path != nil {
